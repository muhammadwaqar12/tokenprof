...
`tokenprof` reads local files and prints to stdout. It makes no network calls, sends no telemetry, and has no runtime dependencies. The optional `tiktoken` extra downloads encoding files on first use, which is the only network activity anywhere in the project.
